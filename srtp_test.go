...
func rtpTestCases() []rtpTestCase {
	return []rtpTestCase{
		{
			sequenceNumber: 5000,
			encryptedCTR:   []byte{0x10, 0x85, 0x4b, 0xfb, 0x4d, 0x70, 0x27, 0x6a, 0x99, 0x19, 0xea, 0x0b, 0x05, 0x4c, 0xeb, 0x49},
			encryptedGCM:   []byte{0x9a, 0x41, 0x9f, 0x44, 0x25, 0xb9, 0xa3, 0x29, 0xc0, 0x5b, 0x30, 0x61, 0x87, 0x86, 0x52, 0x04, 0xf5, 0x3e, 0x0a, 0xd5, 0xd2, 0x4e},
		},
		{
			sequenceNumber: 5001,
			encryptedCTR:   []byte{0x10, 0x85, 0x4b, 0xfb, 0x4d, 0x70, 0x27, 0x6a, 0x99, 0x19, 0xea, 0x0b, 0x05, 0x4c, 0xeb, 0x49},
			encryptedGCM:   []byte{0x9a, 0x41, 0x9f, 0x44, 0x25, 0xb9, 0xa3, 0x29, 0xc0, 0x5b, 0x30, 0x61, 0x87, 0x86, 0x52, 0x4, 0xf5, 0x3e, 0x0a, 0xd5, 0xd2, 0x4e},
		},
		{
			sequenceNumber: 5002,
			encryptedCTR:   []byte{0x10, 0x85, 0x4b, 0xfb, 0x4d, 0x70, 0x27, 0x6a, 0x99, 0x19, 0xea, 0x0b, 0x05, 0x4c, 0xeb, 0x49},
	...
	for name, profile := range profiles {
		...
		t.Run(name, func(t *testing.T) {
			for _, testCase := range rtpTestCases() {
				...
				for i := 1; i < len(encryptedRaw)-16; i++ {
